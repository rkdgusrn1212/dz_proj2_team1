...
const deleteQnA = (id)=>{
     $.ajax({
            ...
            type : "delete",
            success: (response)=> {
                showToast("삭제 성공","방금",id+"번 QnA이 삭제되었습니다");
     ...
     $.ajax({
            ...
            success: (response)=> {//reponse는 count이다.
                ...
                for(let i = pStart; i<= Math.min(pStart+4,response) ; i++){
                     $("#faq-pagination").append("<li class='page-item"+(i==page?" active":"")+"'><a class='page-link' href='javascript:getQnAList("+i+")'>"+i+"</a></li>");
                }
                $("#qna-pagination").append("<li class='page-item"+(pStart+5>response?" disabled":"")+"'><a class='page-link' href='javascript:getQnAList("+(pStart+5)+")'>&raquo;</a></li>");
                ...
                $.ajax({
                    ...
                    success: (response)=> {
                        ...
                        for(; idx < response.length; idx++){
                            ...
                            $("#faq-list").append(
                                    ...
                                    +qna.faqNo+"</th><td class='text-truncate'>"
                                    +qna.faqTitle+"</td><td class='text-truncate'>"
                                    +qna.faqContent+"</td><td class='text-truncate'>"
                                    ...
                                    +"<a style='text-decoration:none;' type='button' href='javascript:showModal("+qna.faqNo+")'><span class='badge rounded-pill bg-danger'>삭제</span></a>"
                                    +"<a style='text-decoration:none;' type='button' href= 'qna/form?id="+qna.faqNo+"'>&nbsp;&nbsp; <span class='badge rounded-pill bg-warning'>수정</span></a></td></tr>");
                        }
                        for(idx ; idx<10; idx++){
                    ...
                    error: ()=>{
                        showToast("QnA 목록 로드 실페","방금","알수 없는 오류 발생");
                    }
                });
            },
            error: ()=>{
                showToast("QnA 페이지네이션 로드 실페","방금","알수 없는 오류 발생");
            }
        });
};
const showModal = (id)=>{
	...
	<div class="container-lg">
		<div class="d-flex justify-content-between mt-5">
			...
			<div class="align-self-end">
				<small><span class='badge rounded-pill bg-danger'>삭제</span>
					버튼을 눌러 해당 QnA을 삭제합니다.&nbsp;&nbsp;&nbsp;<span class='badge rounded-pill bg-warning'>수정</span> 버튼을 눌러 해당 QnA를 수정합니다.</small>
			</div>
		</div>
		...
		<table class="table table-hover table-bordered"
			...
			<thead>
				<tr class="table-primary">
					...
					<th scope="col" class="col-3 text-center text-truncate">QnA 제목</th>
					<th scope="col" class="col-4 text-center text-truncate">QnA 내용</th>
		...
		<div class="row">
			<div class="d-flex justify-content-between align-items-baseline">
				...
				<button class="btn btn-sm btn-success" type="button" onclick = "javascript:location.href = 'qna/form'">새 QnA
					등록</button>
			</div>
		</div>
	</div>
