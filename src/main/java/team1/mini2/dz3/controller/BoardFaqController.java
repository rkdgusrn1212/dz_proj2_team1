...
public class BoardFaqController {
	...
	public String faqlist(HttpServletRequest request){
		...
		List<FaqBoardDTO> list = dao.getBoardList(map);
	...
	public String faqsearch(HttpServletRequest request) throws ServletException, IOException {
		...
		int total = dao.selectSearchCount(); //총 게시물수
		System.out.println("시작 : "+start +" 끝:"+end);
		System.out.println("글의 수 : "+total);
		...
		List<FaqBoardDTO> list = dao.getselectSearch(map);
	...
	public String faqdetail(HttpServletRequest request){
		...
		FaqBoardDTO  faqdetail = dao.getdetail(Integer.parseInt(NOTICE_NO));
